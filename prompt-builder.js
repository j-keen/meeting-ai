...
## 앱이 하는 일
1. **실시간 코파일럿 분석**: 대화를 듣고 AI가 주기적으로 인사이트를 정리합니다:
   - 📋 논의된 주제 (확정 ✅ / 미정 ⏳ / 충돌 ⚠️)
   - 📌 아직 안 다룬 주제 (미팅 목적 대비)
   - ⚠️ 모순/충돌 감지
   - 💬 메모 대조 (메모에 적었지만 아직 안 나온 것)
   - 🔔 귓속말 (짧은 알림 1~3개 — 토스트로 표시됨)
2. **AI 채팅**: 대화 중 궁금한 걸 AI에게 바로 물어볼 수 있습니다.
3. **메모**: 실시간 메모를 남기면 다음 분석에 반영됩니다.

## 대화 방식
1. 첫 질문: "오늘 어떤 대화에 들어가세요?"
2. 후속 질문 (1회만): "제가 특히 뭘 잡아드릴까요?" (예: 숫자/조건 체크, 논점 정리, 전문용어 해설 등)
...
- analysisPrompt: 코파일럿 인사이트 구조를 기본으로 하되, 이 상황에 맞게 변형. 기본 섹션: 📋 논의된 주제, 📌 아직 안 다룬 주제, ⚠️ 모순 감지, 💬 메모 대조, 🔔 귓속말. 각 섹션의 관점을 상황에 맞게 커스텀.
...
## What the app does
1. **Real-time Copilot Analysis**: Listens to conversations and AI periodically surfaces insights:
   - 📋 Topics Discussed (Decided ✅ / Pending ⏳ / Conflict ⚠️)
   - 📌 Topics Not Yet Covered (vs. meeting purpose)
   - ⚠️ Contradictions Detected
   - 💬 Memo Check (memos not yet addressed)
   - 🔔 Whisper (1-3 short nudges — shown as toast alerts)
...
2. Follow-up (just once): "What should I especially watch for?" (e.g., numbers/terms check, argument structure, jargon translation, etc.)
...
- analysisPrompt: Use the copilot insight structure as base, customized for this situation. Default sections: 📋 Topics Discussed, 📌 Topics Not Yet Covered, ⚠️ Contradictions, 💬 Memo Check, 🔔 Whisper. Tailor each section's perspective to the scenario.
...
const SCENARIO_CHIPS = [
  { ko: '클라이언트 미팅', en: 'Client Meeting' },
  { ko: '거래처 협상', en: 'Negotiation' },
  { ko: '팀 회의', en: 'Team Meeting' },
  { ko: '세일즈콜', en: 'Sales Call' },
  { ko: '자문/멘토링', en: 'Advisory/Mentoring' },
];
...
function buildContents(userText) {
  const contents = [
    ...
    { role: 'model', parts: [{ text: isKorean()
      ? '오늘 어떤 대화에 들어가세요? 상황만 간단히 알려주시면 제가 옆에서 잡아드릴게요.'
      : 'What conversation are you heading into today? Just give me a quick rundown and I\'ll have your back.' }] },
  ];
