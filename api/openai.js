// api/openai.js - Vercel Serverless proxy for OpenAI API (with rate limiting)

import { createClient } from '@vercel/kv';

import { OPENAI_ALLOWED_MODELS, resolveModel, isProModel } from '../models.js';
...
const RATE_LIMITS = {
  ...
  pro: { max: 10, windowMs: 60 * 60 * 1000 },       // 10요청/시간
};
...
async function checkRateLimit(ip, model) {
  ...
  const config = isProModel(model) ? RATE_LIMITS.pro : RATE_LIMITS.default;
