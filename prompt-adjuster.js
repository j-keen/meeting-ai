...
// ===== Meta Prompts =====
const META_PROMPT_KO = `당신은 실시간 회의 분석 프롬프트 조정 도우미입니다.

## 역할
사용자가 현재 사용 중인 분석 프롬프트를 보여주면, 사용자의 피드백에 따라 프롬프트를 수정합니다.
...
const META_PROMPT_EN = `You are a real-time meeting analysis prompt adjustment assistant.

## Role
The user shows their current analysis prompt, and you modify it based on their feedback.
...
function updatePromptBar() {
  const bar = $('#paCurrentPromptBar');
  if (!bar) return;
  const prompt = getCurrentPrompt();
  const preview = prompt.length > 100 ? prompt.substring(0, 100) + '…' : prompt;
  bar.querySelector('.pa-prompt-preview').textContent = preview;

  const fullEl = bar.querySelector('.pa-prompt-full');
  fullEl.textContent = prompt;
  fullEl.hidden = true;
...
function buildContents(userText) {
  ...
  const contents = [
    ...
    { role: 'model', parts: [{ text: isKorean()
      ? '현재 프롬프트를 확인했습니다. 어떻게 수정할까요?'
      : 'I\'ve reviewed your current prompt. How would you like to adjust it?' }] },
  ];
...
export function initPromptAdjuster() {
  ...
  // Prompt bar toggle
  $('#paCurrentPromptBar')?.addEventListener('click', () => {
    const bar = $('#paCurrentPromptBar');
    const fullEl = bar.querySelector('.pa-prompt-full');
